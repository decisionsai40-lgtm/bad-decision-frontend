...
export async function POST(req: NextRequest) {
  try {
    ...
    // NGN amounts: 12000 (Starter), 20000 (Growth), 28000 (Pro)
    // USD amounts: 15 (Starter), 25 (Growth), 35 (Pro)
    ...
    if (currency === 'NGN') {
      ...
      if (ngnAmount >= 28000) { purchasedTier = 'pro'; coinsToAdd = 5000 }
        ...
        else coinsToAdd = Math.round(ngnAmount / 8) // Approximate
      }
    } else {
      // USD
      ...
      if (usdAmount >= 35) { purchasedTier = 'pro'; coinsToAdd = 5000 }
        ...
        else coinsToAdd = Math.round(usdAmount * 100) // Approximate
      }
    }
    ...
    if (purchasedTier) {
      ...
      if (tierError) {
        ...
        // Don't fail the whole request — coins were added successfully
      }
    }
