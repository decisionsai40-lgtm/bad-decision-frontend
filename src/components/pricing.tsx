...
export function PricingPage() {
  ...
  const handlePaystackPayment = async () => {
    ...
    // Demo email — in production, get from Clerk auth
    const email = 'user@example.com'
    const amount = isNigeria ? selectedTier.priceNGN : selectedTier.priceUSD
    const currency = isNigeria ? 'NGN' : 'USD'
    const reference = `BD-${Date.now()}-${Math.random().toString(36).slice(2, 8).toUpperCase()}`
    ...
    <div className="min-h-screen bg-white">
      ...
      <Dialog open={paystackOpen} onOpenChange={setPaystackOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPaystackOpen(false)} disabled={paying}>Cancel</Button>
            <Button onClick={handlePaystackPayment} disabled={paying} className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white">
              {paying ? 'Loading Paystack...' : 'Pay with Paystack'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
